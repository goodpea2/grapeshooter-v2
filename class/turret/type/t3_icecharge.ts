...
export class IcechargeAttachedTurret extends AttachedTurret {
  ...
  constructor(type: string, parent: any, hq: number, hr: number) {
    ...
    this.customAssetImg = 't_icecharge_charge1';
  }
  ...
  customUpdate() {
    ...
    if (!isArmed) {
      if (this.staminaSpent > 0) {
        ...
        this.config.actionConfig.pulseBulletTypeKey = 'b_icecharge_explosion';
      }
      this.customAssetImg = 't_icecharge_unarmed';
    ...
    if (this.staminaSpent >= 500) {
      this.customAssetImg = 't_icecharge_charge3';
      this.config.actionConfig.pulseBulletTypeKey = 'b_icecharge_explosion3';
    } else if (this.staminaSpent >= 200) {
      this.customAssetImg = 't_icecharge_charge2';
      this.config.actionConfig.pulseBulletTypeKey = 'b_icecharge_explosion2';
    } else {
      this.customAssetImg = 't_icecharge_charge1';
      this.config.actionConfig.pulseBulletTypeKey = 'b_icecharge_explosion';
    }
...
export class IcechargeWorldTurret extends WorldTurret {
  staminaSpent: number = 0;
...
export const t3_icecharge: TurretConfig = {
  name: 'Ice Charger',
  costs: { sun: 100 },
  costAlmanac: { ice: 16 },
  drops: { ice: 3 },
  ...
  size: 22,
  tier: 3,
  tooltip: "Charges for bigger ice explosion",
  animationBodyType: 'soft',
  actionType: ['pulse'],
  actionConfig: {
    pulseBulletTypeKey: 'b_icecharge_explosion',
  ...
  actionConfigWhileCharged: {
    ...
    staminaSpentThresholds: [
      { staminaSpent: 200, pulseBulletTypeKey: 'b_icecharge_explosion2', assetImg: 't_icecharge_charge2' },
      { staminaSpent: 500, pulseBulletTypeKey: 'b_icecharge_explosion3', assetImg: 't_icecharge_charge3' }
    ]
  },
  targetType: ['enemy'],
