...
export class ActionShootMultiTarget extends TurretAction {
    ...
    if (subStep > 0) {
      ...
      if (state.frames - lastSub >= delay) {
        ...
        if (nextStep > maxCnt) {
           ...
           triggerUpgradeHook('onShot', this.turret, { actionType: 'shootMultiTarget', bulletTypeKey: config.bulletTypeKey });
        }
