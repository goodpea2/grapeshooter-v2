...
export function drawEnemy(e: any) {
  ...
  // We draw the silhouette of all body parts using the outline color
  ...
  // We draw the same shapes but with noStroke to clear internal lines where lobes meet
  noStroke();
  fill(baseCol[0], baseCol[1], baseCol[2]);
  if (e.type === 'e_giant') {
    ellipse(-s * 0.1, -s * 0.2, s * 0.7, s * 0.6);
  ...
  // PASS 3: Internal Details (No Strokes)
  noStroke();
  ...
  if (e.type === 'e_basic' || e.type === 'e_critter') {
    ...
    // Light internal ring to suggest armor without outline
    fill(255, 255, 255, 40);
    ellipse(0, 0, s * 0.7);
    fill(baseCol[0], baseCol[1], baseCol[2]);
    ellipse(0, 0, s * 0.55);
